...
const previewRows = [
  ["Numer zamówienia", "Klient", "Trasa", "Data", "Kierowca", "Status"],
  ["KD/2026/001", "Nazwa klienta", "TR/2026/001", "2026-06-23", "Jan Kierowca", "NEW"],
  ["KD/2026/002", "Nazwa klienta", "TR/2026/001", "2026-06-23", "Jan Kierowca", "NEW"],
];

const importScope = [
  "Order.orderNumber i dane klienta",
  "adres, towar, ilość, palety i waga",
  "Route.routeNumber, planowana data i kierowca",
  "Delivery.deliveryNumber oraz kolejność dostaw w trasie",
];
      ...
      <div className="flex min-h-screen">
        ...
        <section className="flex-1 bg-[radial-gradient(circle_at_top_right,rgba(251,191,36,0.14),transparent_34%),linear-gradient(180deg,#061123_0%,#020813_44%,#020813_100%)]">
          <header className="border-b border-white/10 bg-[#020813]/90 px-4 py-5 shadow-2xl shadow-black/20 backdrop-blur sm:px-6 lg:px-9">
            <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-4 shadow-xl shadow-black/20">
                <p className="text-3xl font-black text-white">{selectedCount}/1</p>
          ...
          <div className="mx-auto w-full max-w-[1600px] space-y-6 px-4 py-5 sm:px-6 lg:p-8">
            ...
            <section>
              <article className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/20 backdrop-blur sm:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-300">
                      Źródło importu
                    </p>
                    <h2 className="mt-2 text-2xl font-black text-white">Import danych</h2>
                    <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-400">
                      Wybierz jeden plik Excel lub CSV zawierający dane zamówień, tras i dostaw. Import obsłuży docelowo cały proces w jednym miejscu zamiast w dwóch osobnych oknach.
                    </p>
                  </div>

                  <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-amber-400/10">
                ...
                <div className="mt-6 rounded-2xl border border-dashed border-white/15 bg-[#020813]/70 p-5 text-center">
                  <input
                    ...
                    className="sr-only"
                  />
                  <label htmlFor="import-data-file" className="mx-auto flex max-w-xl cursor-pointer flex-col items-center rounded-2xl border border-white/10 bg-white/[0.03] px-5 py-8 transition hover:border-amber-400/40 hover:bg-amber-400/10">
                        ...
                        <table className="w-full min-w-[520px] text-left text-xs text-slate-300">
                          <tbody className="divide-y divide-white/10">
                            {previewRows.map((row, index) => (
                              <tr key={`import-${index}`} className={index === 0 ? "text-slate-500" : "text-slate-300"}>
                                {row.map((cell) => (
