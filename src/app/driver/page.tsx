    ...
    <main className="min-h-screen bg-slate-950 p-4 text-white">
      <section className="mx-auto max-w-md rounded-[2rem] border border-white/10 bg-[#07111d] p-5 shadow-2xl shadow-black/50">
        ...
        <div className="mt-6 rounded-2xl bg-white/[0.04] p-5">
          <p className="font-bold">Brak przypisanej trasy</p>
          <p className="mt-1 text-sm text-slate-400">0 punktów dostawy</p>
        ...
        <div className="mt-5 space-y-3">
          {stops.length > 0 ? (
            stops.map(([name, address, status, color], index) => (
              <article key={name} className="rounded-2xl bg-white/[0.03] p-4">
            ...
            <div className="rounded-2xl bg-white/[0.03] p-5 text-center text-sm text-slate-400">
              Brak punktów dostawy do wyświetlenia.
            </div>
          )}
        </div>
