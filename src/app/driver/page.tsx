...
import { ArrowLeft, CheckCircle2, Map, Navigation, Package, Truck } from "lucide-react";
import { Button } from "@/components/ui/button";

const stops = [
  ["Firma ABC Sp. z o.o.", "ul. Przemysłowa 10, Warszawa", "Dostarczono", "text-emerald-400"],
  ["Hurtownia XYZ", "ul. Logistyczna 5, Warszawa", "Dostarczono", "text-emerald-400"],
  ["Sklep 123", "ul. Kwiatowa 20, Warszawa", "W trakcie", "text-amber-300"],
  ["Magazyn DEF", "ul. Towarowa 15, Warszawa", "Oczekuje", "text-slate-400"],
];
    ...
    <main className="min-h-screen bg-slate-950 p-4 text-white">
      <section className="mx-auto max-w-md rounded-[2rem] border border-white/10 bg-[#07111d] p-5 shadow-2xl shadow-black/50">
        ...
        <div className="mt-6 rounded-2xl bg-white/[0.04] p-5">
          ...
          <div className="mt-2 h-2 rounded-full bg-slate-800"><div className="h-2 w-3/5 rounded-full bg-emerald-500" /></div>
        </div>
        <div className="mt-5 space-y-3">
          {stops.map(([name, address, status, color], index) => (
            <article key={name} className={`rounded-2xl p-4 ${index === 2 ? "bg-blue-600/20 ring-1 ring-blue-500/30" : "bg-white/[0.03]"}`}>
              <div className="flex items-start gap-3">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/10 text-sm font-bold">{index + 1}</span>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <Button className="gap-2"><Navigation className="h-4 w-4" /> Nawiguj</Button>
